...
wait_for_service() {
    ...
    log "INFO" "Waiting for $service_name to become available at http://$host:$port/health..." "${BLUE}"
    for (( i=1; i<=$max_attempts; i++ )); do
        # Use curl to check for a successful HTTP response on /health endpoint
        if curl --fail --silent --show-error --output /dev/null "http://$host:$port/health"; then
...
run_server() {
    ...
    cd /app && v python -m uvicorn app:app --host 0.0.0.0 --port "${PORT:-8000}" --reload --reload-dir /app --timeout-graceful-shutdown 0 2>&1 | tee /var/log/server_api.log &
    SERVER_PID=$!
    if [ $? -eq 0 ]; then
        ...
        # Wait for the REST API server to become available (check /health endpoint)
...
#     cd /app && v python -m uvicorn local_api_server:app --host 0.0.0.0 --port 11435 --reload --reload-dir /app --timeout-graceful-shutdown 0 2>&1 | tee /var/log/local_api_server.log &
#     LOCAL_API_PID=$!
#     if [ $? -eq 0 ]; then
...
if [ -f /app/app.py ]; then
    ...
    log "WARNING" "app.py not found in /app directory" "${YELLOW}"
    ls -la /app
