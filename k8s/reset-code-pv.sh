...
kubectl delete pvc code-base -n llmmllab --wait=false
kubectl delete pv code-base --wait=false

# Wait for resources to be deleted
echo "Waiting for resources to be deleted..."
kubectl wait --for=delete pvc/code-base -n llmmllab --timeout=60s || true
...
if kubectl get pvc code-base -n llmmllab 2>/dev/null; then
    echo "Warning: PVC code-base still exists. You may need to force delete it."
    echo "Run: kubectl patch pvc code-base -n llmmllab -p '{\"metadata\":{\"finalizers\":null}}'"
    echo "Then: kubectl delete pvc code-base -n llmmllab --grace-period=0 --force"
else
