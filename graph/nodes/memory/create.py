...
class MemoryCreationNode:
    ...
    async def _create_memories_from_summaries(
        ...
        for summary in summaries:
            ...
            embeddings = await self.embedding_agent.embed([summary.content])
    ...
    async def _create_memories_from_search_syntheses(
        ...
        for synthesis in search_syntheses:
            # Generate embeddings using the injected EmbeddingAgent
            embeddings = await self.embedding_agent.embed([synthesis.synthesis])
    ...
    async def _create_memories_from_documents(
        ...
        for document in documents:
            ...
            # Generate embeddings using the injected EmbeddingAgent
            embeddings = await self.embedding_agent.embed([content])
